...
from thedarn.shells import Fish
...
class TestFish(object):
    ...
    def Popen(self, mocker):
        mock = mocker.patch('thedarn.shells.fish.Popen')
    ...
    @pytest.mark.parametrize('key, value', [
        ...
        ('THEdarn_OVERRIDDEN_ALIASES', 'cut,git,sed'),
        ('THEdarn_OVERRIDDEN_ALIASES', 'cut, git, sed'),
        ('THEdarn_OVERRIDDEN_ALIASES', ' cut,\tgit,sed\n'),
        ('THEdarn_OVERRIDDEN_ALIASES', '\ncut,\n\ngit,\tsed\r')])
    ...
    def test_app_alias(self, shell):
        assert 'function darn' in shell.app_alias('darn')
        assert 'function darn' in shell.app_alias('darn')
        assert 'thedarn' in shell.app_alias('darn')
        assert 'TF_SHELL=fish' in shell.app_alias('darn')
        assert 'TF_ALIAS=darn PYTHONIOENCODING' in shell.app_alias('darn')
        assert 'PYTHONIOENCODING=utf-8 thedarn' in shell.app_alias('darn')
        assert ARGUMENT_PLACEHOLDER in shell.app_alias('darn')

    def test_app_alias_alter_history(self, settings, shell):
        settings.alter_history = True
        assert 'builtin history delete' in shell.app_alias('darn')
        assert 'builtin history merge' in shell.app_alias('darn')
        settings.alter_history = False
        assert 'builtin history delete' not in shell.app_alias('darn')
        assert 'builtin history merge' not in shell.app_alias('darn')
    ...
    def test_put_to_history(self, entry, entry_utf8, builtins_open, mocker, shell):
        mocker.patch('thedarn.shells.fish.time', return_value=1430707243.3517463)
