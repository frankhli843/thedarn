...
import os
import pytest
from thedarn.shells import Bash
...
class TestBash(object):
    ...
    def Popen(self, mocker):
        mock = mocker.patch('thedarn.shells.bash.Popen')
        return mock

    @pytest.fixture(autouse=True)
    def shell_aliases(self):
        os.environ['TF_SHELL_ALIASES'] = (
            'alias darn=\'eval $(thedarn $(fc -ln -1))\'\n'
            'alias l=\'ls -CF\'\n'
            'alias la=\'ls -A\'\n'
            'alias ll=\'ls -alF\'')
    ...
    def test_get_aliases(self, shell):
        assert shell.get_aliases() == {'darn': 'eval $(thedarn $(fc -ln -1))',
                                       'l': 'ls -CF',
                                       'la': 'ls -A',
                                       'll': 'ls -alF'}

    def test_app_alias(self, shell):
        assert 'darn () {' in shell.app_alias('darn')
        assert 'darn () {' in shell.app_alias('darn')
        assert 'thedarn' in shell.app_alias('darn')
        assert 'PYTHONIOENCODING' in shell.app_alias('darn')

    def test_app_alias_variables_correctly_set(self, shell):
        alias = shell.app_alias('darn')
        assert "darn () {" in alias
        assert 'TF_SHELL=bash' in alias
        assert "TF_ALIAS=darn" in alias
