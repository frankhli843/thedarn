import pytest
from thedarn.rules.no_command import match, get_new_command
from thedarn.types import Command


@pytest.fixture(autouse=True)
def get_all_executables(mocker):
    mocker.patch('thedarn.rules.no_command.get_all_executables',
...
def history_without_current(mocker):
    return mocker.patch(
        'thedarn.rules.no_command.get_valid_history_without_current',
...
def test_match(mocker, script, output):
    mocker.patch('thedarn.rules.no_command.which', return_value=None)
...
def test_not_match(mocker, script, output, which):
    mocker.patch('thedarn.rules.no_command.which', return_value=which)
