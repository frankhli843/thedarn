...
import pytest
from thedarn.entrypoints.alias import _get_alias
...
def test_get_alias(monkeypatch, mocker, py2,
    ...
    args = Mock(
        ...
        alias='darn', )
    mocker.patch('thedarn.entrypoints.alias.which', return_value=which)
    shell = Mock(app_alias=lambda _: 'app_alias',
                 instant_mode_alias=lambda _: 'instant_mode_alias')
    monkeypatch.setattr('thedarn.entrypoints.alias.shell', shell)
