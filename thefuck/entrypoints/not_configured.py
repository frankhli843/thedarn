...
def _get_not_configured_usage_tracker_path():
    ...
    return Path(gettempdir()).joinpath(u'thedarn.last_not_configured_run_{}'.format(
        getpass.getuser(),
    ))
...
def _is_second_run():
    """Returns `True` when we know that `darn` called second time."""
    ...
    return (_get_previous_command() == 'darn' or
...
def main():
    ...
    It'll be only visible when user type darn and when alias isn't configured.
