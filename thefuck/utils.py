...
class Cache(object):
    ...
    def _setup_db(self):
        ...
        cache_path = Path(cache_dir).joinpath('thedarn').as_posix()
...
def get_installation_info():
    ...
    return pkg_resources.require('thedarn')[0]


def get_alias():
    return os.environ.get('TF_ALIAS', 'darn')


@memoize
def get_valid_history_without_current(command):
    def _not_corrected(history, tf_alias):
        """Returns all lines from history except that comes before `darn`."""
        previous = None
        for line in history:
